...
var Parser = function() {
    ...
    this.parse = function(stream) {
        ...
        if (!T.next())
            ...
            throw new Error("more that one root");
    };
            ...
            if (T.get() === TOKEN.ANGCLOSE) {
                ...
                if (T.get() === TOKEN.TAGCLOSE1) {
                    ...
                    while (T.get() !== TOKEN.TAGCLOSE1) {
                        var val = jsxel(node);
                        if (val.type) { // here we can recognise if is JS or a string
...
var jsx = '<span><span test= "sdas" asdsa={}><p /> <p>"ahs"</p></span></span>';

p = new Parser();
var stream = new Stream(jsx);
console.log(JSON.stringify(p.parse(stream)));
